...
int	rendering_function(void *param)
{
	...
	while (++x < W_WIDTH)
	{
		...
		while (hit == 0)
		{
			...
			if (gd->map[map_pos.y][map_pos.x] > 0) hit = 1;
		}
		//Calculate distance projected on camera direction. This is the shortest distance from the point where the wall is
		//hit to the camera plane. Euclidean to center camera point would give fisheye effect!
		//This can be computed as (mapX - gd->player.pos.x + (1 - stepX) / 2) / rayDirX for side == 0, or same formula with Y
	...
	gd->frame_time = (gd->time - gd->old_time) / 10000000.0; //frameTime is the time this frame has taken, in seconds
	// printf("FPS = %f\n", 1.0 / frameTime); //FPS counter

	mlx_put_image_to_window(gd->mlx, gd->win, gd->canvas.img, 0, 0);

	//speed modifiers
	double moveSpeed = gd->frame_time * 5.0 * 8; //the constant value is in squares/second
	double rotSpeed = gd->frame_time * 3.0 * 4; //the constant value is in radians/second

	//move forward if no wall in front of you
	if(gd->keys[KEY_UP])
	{
		//if (gd->map[(int)gd->player.pos.y][(int)(gd->player.pos.x + gd->dir.x * moveSpeed)] == false)
			gd->player.pos.x += gd->dir.x * moveSpeed;
		//if (gd->map[(int)(gd->player.pos.y + gd->dir.y * moveSpeed)][(int)gd->player.pos.x] == false)
			gd->player.pos.y += gd->dir.y * moveSpeed;
		/*
		if (gd->map[(int)(gd->player.pos.y + gd->dir.y * moveSpeed)][(int)(gd->player.pos.x)] == false)
			gd->player.pos.y += gd->dir.y * moveSpeed;
		if (gd->map[(int)gd->player.pos.y][(int)(gd->player.pos.x + gd->dir.y * moveSpeed)] == false)
			gd->player.pos.y += gd->dir.y * moveSpeed;
		*/
	}
	...
	if(gd->keys[KEY_A])
	{
		...
		if (gd->map[(int)gd->player.pos.y][(int)(gd->player.pos.x + cos_val * moveSpeed)] == false)
			gd->player.pos.x += cos_val * moveSpeed;
		if (gd->map[(int)(gd->player.pos.y + sin_val * moveSpeed)][(int)gd->player.pos.x] == false)
